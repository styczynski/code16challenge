...
const gridOffsetTop = -10;
const gridOffsetLeft = -10;
const cloudInterval = 10;
const sunSize = 20;
const sunLightShadowCover = 100;
const sunLightShadowSharpness = 0.7;
const sunLightShadowSharpnessX = 1.5;
const sunLightShadowSharpnessY = 1.5;
const groundY = 25;
const sunOuterSize = 60;
const sunPosition = {
  x: 20,
  y: 5
};
...
const setupGrid = function() {
  gridSize = {
    w: Math.ceil(1000 / gridBlockSize),
    h: Math.ceil(1000 / gridBlockSize)
  };
  txt="<div id=\"view\" style=\"width:800px;height:200px\"><div id=\"grid\"/>";
  eachGridBlock((x, y) => txt+=`<div id=\"block${x}x${y}\" class=\"grid-block\" data-coords=\"${x}x${y}\" style="width:10px;height:10px;position:absolute;left:${gridOffsetLeft + (x*gridBlockSize)}px;top:${gridOffsetTop + (y*gridBlockSize)}px;"></div>`);
...
const drawGround = function() {
  ...
  for (let x = 0, end = gridSize.w, asc = 0 <= end; asc ? x <= end : x >= end; asc ? x++ : x--) {
    ...
    for (let y = groundH, end1 = gridSize.h+10, asc1 = groundH <= end1; asc1 ? y <= end1 : y >= end1; asc1 ? y++ : y--) {
      let fi = ((((y-groundY)+7)*2) + (Math.random()*4) + (Math.sin(x/10)*10)) - ((sunLightShadowCover-Math.min(Math.pow(Math.abs(x-sunPosition.x), sunLightShadowSharpnessX)+(2*Math.pow(Math.abs(y-groundY), sunLightShadowSharpnessY)), sunLightShadowCover))*sunLightShadowSharpness);
      fi /= 1;
...
const drawSun = function() {
  ...
  let asc2, end2, start;
  const shape = {};
  for (x = -sunSize, end = sunSize, asc = -sunSize <= end; asc ? x <= end : x >= end; asc ? x++ : x--) {
    var asc1, end1;
    for (y = -sunSize, end1 = sunSize, asc1 = -sunSize <= end1; asc1 ? y <= end1 : y >= end1; asc1 ? y++ : y--) {
      if (((x*x) + (y*y)) <= (sunSize/2)) {
        let fi = Math.ceil((x+y)*7);
        if (Math.abs(Math.floor(((x*x) + (y*y)) - (sunSize/2))) < 2) {
          if (Math.random() > 0.2) {
            fi*=-0.5;
          }
        }
  ...
  for (start = -sunSize-1, x = start, end2 = sunSize+1, asc2 = start <= end2; asc2 ? x <= end2 : x >= end2; asc2 ? x++ : x--) {
    var asc3, end3, start1;
    for (start1 = -sunSize-1, y = start1, end3 = sunSize+1, asc3 = start1 <= end3; asc3 ? y <= end3 : y >= end3; asc3 ? y++ : y--) {
      if ((((x*x) + (y*y)) <= (sunOuterSize/2)) && ( !(((x*x) + (y*y)) <= (sunSize/2)) )) {
...
const drawingLoop = function() {
  ...
  if (remCounter > 0) {
    return __range__(0, remCounter-1, true).map((j) =>
      addNewCloud( -20+(j*cloudInterval*Math.random()) ));
  }
};
