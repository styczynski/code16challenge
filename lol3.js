...
const eachGridBlock = fn => __range__(0, 100, true).map((x) =>
  __range__(0, 100, true).map((y) =>
    fn(x, y)));
  ...
  const baloon = {
    ...
    shapefn() {
      if (this.speedY === 0) {
        if (Math.random() > 0.5) {
          if (Math.random() > 0.4) {
            this.speedY = dd((Math.random()*1)+1);
          } else {
            this.speedY = -1;
          }
        }
...
const drawGround = function() {
  ...
  for (let x = 0, end = 100, asc = 0 <= end; asc ? x <= end : x >= end; asc ? x++ : x--) {
    ...
    for (let y = groundH, end1 = 100+10, asc1 = groundH <= end1; asc1 ? y <= end1 : y >= end1; asc1 ? y++ : y--) {
      let fi = ((((y-25)+7)*2) + (Math.random()*4) + (Math.sin(x/10)*10)) - ((100-Math.min(Math.pow(Math.abs(x-20), 1.5)+(2*Math.pow(Math.abs(y-25), 1.5)), 100))*0.7);
      fi /= 1;
