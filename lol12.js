t=x=>~~x,n=Math.random,e=[],r=[],T=-75,c=-20,Y=-c,f=255,A=.9,s=25,u=Math.sin;let h=[[[46,139,87],[46,139,87]],[[238,59,59],[232,232,232]],
...
top:${10*o-10}px;"></div>`),document.write(O+"</div>")},d=o=>{let r=~~(n()*h.length);r+=h.length;let Z=h[r%=h.length][0],f=h[r][1],i={x:~~(o+2*n()),y:~~(4*n()),d:2,c:0,w(){
...
n()>.6&&a.push([e,r,[99,184,f]]);q(Y,5,a)}(),function(){let t=[];for(let o=0, e=100,i=0<=e;i?o<=e:o>=e;i?o++:o--){let e=r[o];for(let r=e,i=110,h=e<=i;h?r<=i:r>=i;h?r++:r--){t.push
([o,r,[152,f,152]])}}q(0,0,t)}();for(let t of Array.from(e))null!=t.w&&(t.o=t.w(t)),q(t.x,t.y,t.o),t.x+=t.d,t.a=0,t.b=t.d,null!=t.c?(t.y+=t.c,t.a=t.c):n()>A&&t.y<=3&&t.y>=0 ?n()>.5?
(++t.y,t.a=1):(--t.y,t.a=-1):t.y>=3?(--t.y,t.a=-1):(++t.y,t.a=1);let i=e.length-1,h=0;for(;i>=0;)e[i].x>100&&(e.splice(i,1),++h),--i;if(h>0)for(j=0;j<h;++j)g(c+10*j*n())};y(),
r=[s,26,27];for(let C=0,e=100,c=0<=e;c?C<=e:C>=e;c?C++:C--){let I=r.length;n()>A?(D=(D=s+3*n())+r[I-1]+r[I-2]+r[I-3],D/=4):D=n()>A?s+r[I-1]-r[I-2]:r[I-1],r.push(D)}E=()=>r.shift(),E()
